...
def run(task, input_concat, model_type, i):
    ...
    if input_concat == True:
        ...
        model.save_weights(f"saved/{task}/3_ego/concat/{model_type}/{task}_ego_concat_{model_type}_{i}.h5", save_format="h5")
        ...
        model.save_weights(f"saved/{task}/3_ego/both/{model_type}/{task}_ego_both_{model_type}_{i}.h5", save_format="h5")
    ...
    for input_concat in input_concat_list:
        for model_type in model_type_list:
            ...
            for i in range(repeat_num):
                history, loss, accuracy= run(task, input_concat, model_type, i)
...
df_new.to_csv("saved/3_ego_multiclass_rnn.csv", index=False)
